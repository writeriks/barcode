...
- **Yahoo Shopping** — when a Yahoo Shopping application ID is configured *and* the app language is Japanese, the barcode is also sent to [Yahoo! JAPAN's shopping search API](https://developer.yahoo.co.jp/webapi/shopping/) so a Japanese listing (name, photo, price) can fill in when the open databases have little or nothing. Other languages never call this API. See Yahoo's own developer terms for how they handle that request.
- **Taobao** — only if a lookup proxy URL is configured in the build *and* the app language is Chinese. In that case the barcode is sent to that proxy so it can query Taobao's barcode API on our behalf. The app never stores Taobao credentials. If that URL is not configured, or the UI is not Chinese, nothing is sent to Taobao.
